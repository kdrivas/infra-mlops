...
def feature_engineering(path: str, dry_run: bool = False) -> None:
    ...
    df_merge["Periodo"] = df_merge.apply(
        lambda x: str(int(x.anio)) + "-" + str(int(x.mes)), axis=1
    )
    ...
    df_prec = pipe_1.fit_transform(
        df_merge.drop(TARGET_COL, axis=1), df_merge[TARGET_COL]
    )
    ...
    df_prec_train = pd.concat(
        (df_interm_y_train, pd.DataFrame(df_interm_X_train)), axis=1
    )
    ...
    if not dry_run:
        ...
        joblib.dump(pipe_1, os.path.join(ARTIFACT_DIR, "data_pipeline.pkl"))
